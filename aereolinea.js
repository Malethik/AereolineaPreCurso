let userName = prompt("Inserir nombre usuario");
alert(`Bienvenido ${userName}, gracias por usar nuestro servicio!`)
...
while (true) {
    ...
    let level = prompt("Selecionar una opcion: user, admin, exit.");

    const access = () => {
        switch (level) {
            case ("admin"):
                let flightToDeleteId = parseInt(prompt("ingresar el ID del vuelo a eliminar."))
                let indexToDelete = flight.findIndex(finded => finded.id === flightToDeleteId);
                if (indexToDelete !== -1) {
